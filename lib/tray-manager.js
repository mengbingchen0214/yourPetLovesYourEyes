...
function buildMenu(ctx) {
  ...
  return Menu.buildFromTemplate([
    ...
    ...(license && !license.isPro() ? [{ type: 'separator' }, {
      label: `⭐ 升级到 Pro (${license.isTrial() ? license.getTrialInfo().daysRemaining + '天试用' : '解锁全部功能'})`,
      click: () => {
        if (ctx.notifyRenderer) ctx.notifyRenderer('upgrade-prompt', { feature: 'pro-upgrade' });
      }
    }] : []),
    { type: 'separator' },
    { label: 'Quit 退出', click: () => {
