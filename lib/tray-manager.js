...
function createTray(ctx) {
  console.log('[Tray DEBUG] createTray called');
  if (!Tray || !nativeImage || !app) {
    console.error('[Tray DEBUG] Missing required modules:', { Tray: !!Tray, nativeImage: !!nativeImage, app: !!app });
    return;
  }

  const iconPath = path.join(__dirname, '..', 'assets', 'icon.png');
  console.log('[Tray DEBUG] Icon path:', iconPath);
  console.log('[Tray DEBUG] Icon exists:', fs.existsSync(iconPath));
  let icon;

  if (fs.existsSync(iconPath)) {
    icon = nativeImage.createFromPath(iconPath);
    console.log('[Tray DEBUG] Icon created, isEmpty:', icon.isEmpty(), 'size:', icon.getSize());
  } else {
    console.error('[Tray DEBUG] Icon file not found!');
  ...
  if (!icon.isEmpty()) {
    ...
    console.log('[Tray DEBUG] Icon processed, final size:', icon.getSize());
  }

  try {
    tray = new Tray(icon);
    console.log('[Tray DEBUG] Tray object created successfully');
  } catch (err) {
    console.error('[Tray DEBUG] Failed to create Tray:', err.message);
    return;
  }
...
function buildMenu(ctx) {
  ...
  return Menu.buildFromTemplate([
    ...
    { type: 'separator' },
    { label: '☕ 支持开发者', click: () => ctx.showDonation() },
    { type: 'separator' },
    { label: 'Quit 退出', click: () => {
      app.isQuitting = true;
      app.quit();
